...
FROM employees, departments;  #查询出2289条记录
...
#4.如果查询语句中出现了多个表中都存在的字段，必须指明此字段所在的表。
...
#SLQ99语法中使用 JOIN ... ON 的方式实现多表的查询。这种方式也能解决外连接的问题。MySQL是支持此种方式的。
#SQL99语法如何实现多表的查询。
...
#UNION：会执行去重操作
#UNION ALL：不会执行去重操作
#结论：如果明确知道合并数据后的结果数据不存在重复的数据，或者不需要去除重复的数据，则尽量使用UNION ALL语句，以提高数据查询的效率。
...
#NATURAL JOIN：它会帮你自动查询两张连接表中的所有相同的一段，然后进行等值连接。
SELECT employee_id, last_name, department_name
FROM employees e
